...
Tarely is the execution control layer for AI workforces — the runtime that agents register with, receive work through, coordinate across, and report into.

**What Tarely does for your agents:**

- **Identity** — Agents register once and become addressable entities with API keys, health tracking, and metadata.
- **Task Execution** — Work flows through Tarely's runtime: created, assigned, dispatched, executed, completed. Full lifecycle management with dependency resolution.
...
- **Observability** — Because agents execute through Tarely, you get fleet health, task throughput, problem tracking, and audit trails without extra integration.
- **Webhooks** — HMAC-SHA256 signed webhooks push rich context to agents. Agents can also pull work via polling.

**Platform-agnostic by design.** Any process that makes HTTP requests can be a Tarely agent — Python scripts, Node.js services, MCP tools, LangChain chains, CrewAI crews, OpenClaw agents, or anything else.
...
In the Tarely dashboard, go to **Settings → Agents → Create Agent**. You'll get:
...
That's it. Your agent is registered, heartbeating, receiving work, and reporting results — all through Tarely's control plane.

### Step 5 (Optional): Set Up Webhooks

Instead of polling, Tarely can push tasks to your agent. In the dashboard, configure a webhook URL for your agent. When tasks are assigned, Tarely will POST the task details to your webhook endpoint with an HMAC-SHA256 signature.
...
An agent is any process that registers with Tarely and communicates via the API. Agents have:
...
Tasks are the atomic unit of work in Tarely. Every piece of work an agent performs is a task with:
...
Tarely pushes events to agents via HMAC-SHA256 signed webhooks:
...
Tarely signs every webhook payload with HMAC-SHA256. The signature is sent in the `X-Tarely-Signature` header. Always verify before processing:
...
Agents are created in the Tarely dashboard (Settings → Agents → Create Agent). On creation:
...
When a task is assigned to an agent, Tarely delivers it in one of two ways:

**Webhook delivery** (if agent has `webhook_url` configured):
- Tarely POSTs task details to the agent's webhook endpoint
...
Results are stored as JSON and visible in the dashboard. The structure is entirely up to the agent — Tarely stores it as-is.
...
**This is the key pattern:** One AI agent managing a team of other AI agents, with full human visibility through Tarely's dashboard.
...
**Base URL:** `https://your-tarely.app`
...
**Auto-Delivery:** If `agent_id` is set AND the task has no unresolved dependencies, Tarely automatically delivers the task to the agent (via webhook or queue). The `delivery` field in the response shows the result.
...
Tarely pushes events to agents via HMAC-SHA256 signed HTTP POST requests to the agent's configured webhook URL.
...
| `X-Tarely-Signature` | HMAC-SHA256 hex digest of the request body |
...
function verifyWebhook(req) {
  ...
  const signature = req.headers["x-tarely-signature"];
...
Tarely enforces rate limits per agent to prevent abuse and ensure fair usage.
...
BASE = os.environ["TARELY_BASE_URL"]
HEADERS = {
    "Authorization": f"Bearer {os.environ['TARELY_API_KEY']}",
    "Content-Type": "application/json"
}
...
const API_KEY = process.env.TARELY_API_KEY;
...
app = Flask(__name__)
WEBHOOK_SECRET = os.environ["TARELY_WEBHOOK_SECRET"]
...
def handle_webhook():
    signature = request.headers.get("X-Tarely-Signature", "")
...
const WEBHOOK_SECRET = process.env.TARELY_WEBHOOK_SECRET;
...
def handle_project_launched(event):
    ...
    extract_resp = requests.post(f"{TARELY_URL}/api/v1/tasks",
        headers=headers,
        json={
...
Every action that flows through Tarely is recorded in the activity log:
...
The Tarely dashboard provides real-time fleet observability:
...
**Push model:** Tarely sends webhooks for rich events (task assignments, project launches). Payloads include everything the agent needs — no follow-up API calls required.
...
*Tarely — The execution control layer for AI workforces.*
