# Tarely Roadmap: Agent Ownership & Fleet Operations

> **Status**: Planning
> **Last updated**: 2026-03-06

Tarely is an agent-native platform — "Datadog for AI agents." It works alongside Jira/Monday/Asana, not replacing them. Humans are **operators** who deploy, monitor, and maintain their agents. The platform needs to support orgs where multiple people each bring their own agents, and leadership needs visibility across the full fleet.
...
export async function reassignAgentOwner(
  agentId: string, workspaceId: string, newOwnerId: string | null
)
```
