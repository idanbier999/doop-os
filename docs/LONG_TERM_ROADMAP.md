...
Doop today is a functional execution control plane with:
...
**Why it matters:** Real-time toasts in the dashboard are great when you're looking. When an agent goes critical at 3 AM, the operator needs a Slack ping. Without external alerting, Doop is a monitoring tool you have to actively watch — that defeats the purpose of autonomous agents.
...
A developer should go from "what is Doop?" to "my agent is heartbeating" in under 10 minutes.
...
- A developer with no prior Doop knowledge can connect an agent in <10 minutes
- Code examples are copy-pasteable and work against a real Doop instance
...
| Tasks executed      | 1,000+ | Doop is a real control plane, not just a monitoring dashboard |

All three metrics must be hit. Companies without active agents means shelfware. Agents without tasks means passive monitoring. Tasks without scale means a toy. The combination proves Doop is infrastructure that companies operate through.
...
- Python package (`doop`) and Node package (`@doop/sdk`)
...
**Why it matters:** The REST API is deliberately minimal, but even minimal APIs have boilerplate: auth headers, JSON parsing, error handling, heartbeat scheduling. SDKs eliminate that boilerplate. The goal is `pip install doop` -> 5 lines of code -> agent is running.

**Completion criteria:**

- `pip install doop` and `npm install @doop/sdk` work
...
**Goal:** Create network effects and accelerate adoption. This phase moves Doop from "tool" to "ecosystem."
...
A catalog of ready-to-run agents that work with Doop out of the box.

**Core needs:**

- Marketplace page: browse, filter, deploy pre-built agents
- Partner integrations: third-party agents that register with Doop natively
- Standardized agent packaging format

**Why it matters:** When a company can pick a "Customer Support Triage Agent" from a marketplace and have it heartbeating in 5 minutes — that's when Doop becomes a platform. Network effects begin when agents attract users and users attract builders.
...
2. **SDK scope creep.** SDKs must stay minimal. Three methods, zero framework dependency. The moment the SDK becomes a framework, it undermines Doop's platform-agnostic positioning.
...
The roadmap prioritizes getting Doop into real hands fast:
