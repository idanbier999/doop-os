...
describe("useRealtime", () => {
  ...
  it("subscribes to realtime channel on mount", () => {
    ...
    renderHook(() => useRealtime({ table: "tasks", onPayload }));
  ...
  it("calls supabase.channel with correct channel name", () => {
    ...
    renderHook(() => useRealtime({ table: "agents", onPayload }));
  ...
  it("passes filter to channel config when provided", () => {
    ...
    expect(mockSupabase.channel).toHaveBeenCalledWith("realtime-tasks-workspace_id=eq.ws-001");
    expect(mockChannel.on).toHaveBeenCalledWith(
      "postgres_changes",
  ...
  it("cleans up channel on unmount (calls removeChannel)", () => {
    ...
    const { unmount } = renderHook(() => useRealtime({ table: "tasks", onPayload }));
