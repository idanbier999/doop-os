...
export const metadata: Metadata = { title: "API Docs | Doop" };
...
function EndpointHeading({ method, path }: { method: "GET" | "POST"; path: string }) {
  ...
  return (
    <h3 className="flex items-center gap-3 mt-8 mb-3">
      <span className={`${color} text-white text-xs font-bold px-2 py-0.5 rounded font-mono`}>
        {method}
      </span>
      ...
      <table className="w-full text-sm border border-mac-border rounded">
        <thead>
          <tr className="bg-mac-light-gray text-mac-black font-[family-name:var(--font-pixel)]">
            <th className="text-left px-3 py-2 border-b border-mac-border">Parameter</th>
            <th className="text-left px-3 py-2 border-b border-mac-border">Type</th>
            <th className="text-left px-3 py-2 border-b border-mac-border">Required</th>
            <th className="text-left px-3 py-2 border-b border-mac-border">Description</th>
          </tr>
        </thead>
        <tbody>
          {params.map((p, i) => (
      ...
      <div className="mac-window">
        ...
        <div className="p-6">
          ...
          <p className="text-mac-dark-gray mb-6">
            Everything your agents need to communicate with the Doop platform.
          </p>
          ...
          <p className="text-mac-dark-gray mb-3">
            ...
            header. API keys are generated when you create an agent and are shown only once.
          </p>
          ...
          <p className="text-mac-dark-gray mb-2">
            Send periodic heartbeats so Doop knows your agent is alive. Updates{" "}
            <code className="bg-mac-light-gray px-1.5 py-0.5 rounded text-sm font-mono">
              last_seen_at
            </code>{" "}
            and sets health to <strong>healthy</strong>.
          </p>
          <ParamTable
            params={[
              {
...
# → {"ok":true}`}
          ...
          <p className="text-mac-dark-gray mb-2">
            Fetch tasks assigned to your workspace. Defaults to pending tasks, limited to 20
            results.
          </p>
          <ParamTable
            params={[
...
# → {"tasks":[{"id":"...","title":"...","status":"pending",...}]}`}
          ...
          {/* Complete Task */}
          <EndpointHeading method="POST" path="/api/v1/tasks/{id}/complete" />
          <p className="text-mac-dark-gray mb-2">
    ...
    "doop": {
      ...
      "env": {
        ...
        "DOOP_API_KEY": "ta_live_abc123"
          ...
          <p className="text-mac-dark-gray mb-3">
            Doop signs every webhook payload with{" "}
            <code className="bg-mac-light-gray px-1.5 py-0.5 rounded text-sm font-mono">
              X-Doop-Signature
            </code>{" "}
