...
describe("GET /api/activity/export", () => {
  ...
  it("returns 403 when not a workspace member", async () => {
    // db.select() for workspace_members → empty (no membership)
    pushResult([]);
  ...
  it("returns 400 for invalid from date", async () => {
    // db.select() for workspace_members → authorized
    pushResult([{ id: "m-1" }]);
  ...
  it("returns empty JSON export when no rows", async () => {
    // db.select() for workspace_members → authorized
    pushResult([{ id: "m-1" }]);
    // db.select() for activity_log → empty
    pushResult([]);
  ...
  it("returns JSON export with proper headers", async () => {
    ...
    // db.select() for activity_log → rows
    pushResult(rows);
    ...
    expect(body[0].timestamp).toBe("2024-06-01T12:00:00.000Z");
  ...
  it("returns 400 when too many results (>10000)", async () => {
    ...
    // db.select() for activity_log → overflow rows
    pushResult(overflowRows);
