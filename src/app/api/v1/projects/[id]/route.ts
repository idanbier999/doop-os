...
async function handleGet(request: NextRequest, context?: unknown) {
  ...
  const { data: project, error: projectError } = await supabase
    .from("projects")
    .select("id, name, description, instructions, orchestration_mode, status, created_at, updated_at")
    .eq("id", id)
  ...
  const { data: teamRaw } = await supabase
    .from("project_agents")
    .select("role, status, agent:agents!project_agents_agent_id_fkey(id, name, capabilities, agent_type, health, webhook_url)")
    .eq("project_id", id);
