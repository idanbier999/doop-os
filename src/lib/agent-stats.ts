...
export async function getAgentStats(
  ...
  const [
    ...
    supabase.from("tasks").select("id, status").eq("agent_id", agentId),
    ...
    supabase.from("problems").select("id, status").eq("agent_id", agentId),
  ...
  const problemRate = completedCount > 0 ? problems.length / completedCount : 0;
