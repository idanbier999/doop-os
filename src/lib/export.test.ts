...
describe("toExportEntries", () => {
  ...
  it("maps created_at to an ISO timestamp", () => {
    const entries = toExportEntries([makeEntry({ created_at: "2026-02-27T12:00:00Z" })]);
  ...
  it("uses agent name from joined agents object", () => {
    const entries = toExportEntries([makeEntry({ agents: { name: "MyAgent" } })]);
