...
export function ProblemsTable({ initialProblems, agents }: ProblemsTableProps) {
  const [problems, setProblems] = useState<ProblemWithAgent[]>(initialProblems);
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <EmptyState message="No problems reported" description="Your agents are running smoothly" />
      ) : (
            ...
            <Tbody>
              {filtered.map((problem) => (
                <Tr key={problem.id}>
                  ...
                  <Td>
                    <span className="font-medium text-mac-black">{problem.title}</span>
                    {problem.description && (
                  ...
                  <Td className="whitespace-nowrap">{formatDate(problem.created_at)}</Td>
                  <Td>
                    <ProblemActions
