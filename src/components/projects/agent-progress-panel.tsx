...
function AgentCard({ info }: { info: ProjectAgentInfo }) {
  ...
  return (
    <div className={`border rounded-lg p-4 bg-mac-white ${roleBorderColors[info.role]}`}>
      {/* Header */}
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          />
          <span className="text-sm font-bold text-mac-black truncate">{info.agent.name}</span>
        </div>
      ...
      <div className="mb-3">
        ...
        {info.currentTask ? (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-mac-dark-gray truncate">{info.currentTask.title}</span>
          </div>
        ) : (
      ...
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          <p className="text-xs text-mac-gray font-[family-name:var(--font-pixel)]">Tasks</p>
          ...
          <ul className="space-y-1">
            {info.recentActivity.slice(0, 5).map((activity) => (
              <li key={activity.id} className="flex items-center justify-between gap-2">
                <span className="text-xs text-mac-dark-gray truncate">{activity.action}</span>
          ...
          prev.map((a) =>
            a.id === updated.id
              ? {
                  ...a,
                  ...(updated.status
                    ? { status: updated.status as ProjectAgentInfo["status"] }
                    : {}),
                }
              : a
          )
