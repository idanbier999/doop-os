...
describe("OperatorFleetSummary", () => {
  it("renders one card per operator", () => {
    render(<OperatorFleetSummary operators={mockOperators} onSelectOperator={vi.fn()} />);
  ...
  it("returns null when operators.length <= 1", () => {
    const { container } = render(
      <OperatorFleetSummary operators={[mockOperators[0]]} onSelectOperator={vi.fn()} />
    );
  ...
  it("calls onSelectOperator when card clicked", () => {
    const handler = vi.fn();
    render(<OperatorFleetSummary operators={mockOperators} onSelectOperator={handler} />);
