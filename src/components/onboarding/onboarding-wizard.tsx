              ...
              <div key={label} className="flex items-center gap-2">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-medium ${
                    i <= step ? "bg-blue-600 text-white" : "bg-mac-white text-mac-dark-gray"
                  }`}
                >
                  {i + 1}
                </div>
                <span className={`text-sm ${i <= step ? "text-mac-black" : "text-mac-gray"}`}>
                  {label}
                </span>
                {i < steps.length - 1 && (
                  <div className={`h-px w-8 ${i < step ? "bg-blue-600" : "bg-mac-white"}`} />
                )}
              </div>
            ))}
          </div>
