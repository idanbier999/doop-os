...
export function AgentsSettings() {
  ...
  async function handleGenerateSecret(agentId: string) {
    ...
    if (!error) {
      ...
      addToast({ type: "info", title: "Webhook secret generated", description: "New secret saved for agent" });
    }
  }
  ...
  async function handleTestWebhook(agentId: string) {
    ...
    if (result.success) {
      addToast({ type: "info", title: "Webhook test succeeded", description: "The endpoint responded successfully" });
    } else {
  ...
  async function handleSaveCapabilities(agentId: string) {
    const caps = capabilitiesInput.split(",").map((c) => c.trim()).filter(Boolean);
    const { error } = await supabase
      .from("agents")
    ...
    if (!error) {
      setAgents((prev) =>
        prev.map((a) => a.id === agentId ? { ...a, capabilities: caps } : a)
      );
    }
                              ...
                              {editingWebhook === agent.id ? (
                                <div className="flex items-center gap-2">
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => { setEditingWebhook(null); setWebhookInput(""); }}
                                  >
                                    Cancel
                                  </Button>
                                </div>
                              ...
                              {editingCapabilities === agent.id ? (
                                <div className="flex items-center gap-2">
                                  ...
                                  <Button
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => { setEditingCapabilities(null); setCapabilitiesInput(""); }}
                                  >
                                    Cancel
                                  </Button>
                                </div>
